...
run "compute_disabled_by_default" {
  command = plan
  assert {
    condition     = output.resource_group_name == "rg-costtest-prod" && output.deployment_summary.environment == "prod"
    error_message = "PROD deve propagar seu ambiente e gerar nomes próprios."
  }
  assert {
